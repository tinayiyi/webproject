  ...
  <div>
    ...
    >
      <div class="chongfubox">
        ...
        <el-table :data="ThisTimeSubData" border style="width: 100%" stripe>
              ...
              <div class="cardtbbox">
                <el-form label-position="left" inline class="demo-table-expand">
                  ...
                  <el-form-item label="所在城市:">
                    <span>{{ props.row.details.position.AbroadCity }}</span>
                  </el-form-item>
          ...
          <el-table-column label="主讲人" prop="maininfo.speaker" width="140"> </el-table-column>
              ...
              <template slot-scope="props">
                <div class="cardtbbox">
                  <el-form label-position="left" inline class="demo-table-expand">
                    ...
                    <el-form-item label="标签:">
                      <span
                        v-show="props.row.maininfo.type == item.id"
      ...
      <span slot="footer" class="dialog-footer">
        <el-button @click="chongfuquxiao">取消提交</el-button>
    ...
    >
      <CommonForm
        ...
        @isForm="isForm"
      />
      <div slot="footer" class="dialog-footer" v-show="isfootershow">
    ...
    <el-card>
      ...
      >
            ...
            <div class="cardtbbox">
              <el-form label-position="left" inline class="demo-table-expand">
                ...
                <!-- <el-form-item label="主办方Logo:">
                  <img :src="props.row.maininfo.logo" class="imglogo" />
                </el-form-item> -->
                <el-form-item label="封面:">
                  <img :src="props.row.maininfo.coverUrl" class="imgcover" />
                </el-form-item>
        ...
        <el-table-column class="biaoti" label="标题" prop="maininfo.title"> </el-table-column>
        <el-table-column class="biaoti" label="国外" prop="details.position.isAbroad" width="50">
          <template slot-scope="scope">
            <span v-show="scope.row.details.position.isAbroad == true">是</span>
          </template>
        </el-table-column>
        <el-table-column class="biaoti" label="添加时间" prop="details.else.addTime" width="130"> </el-table-column>
        <el-table-column class="biaoti" label="操作人" prop="details.else.name" width="130">
...
import { addData, getMeetList, editData, deleteData, getSelectList, getHideData, changeHideType } from '../../api/main'
  ...
  data() {
    return {
      ...
      operateForm: {
        ...
        details: {
          price: '',
          position: { address: [], xxaddress: '', isAbroad: false, AbroadCity: '' },
          describe: [{ title: '', text: [''] }]
        },
        industry: ''
      },
      ...
      forceData: [
        {
          starTT: '2021-04-26 15:14:47',
          endTT: '2021-04-26 15:14:47',
          maininfo: {
            mid: 2393,
            oid: '',
            logo: '',
            tags: '',
            type: 505,
            title: '2021年郑州食品饮料机械设备展览会',
            endTime: 1619604000000,
            hotMeet: true,
            orgName: '河南省肉类协会、河南瑞城文化传媒集团',
            speaker: '',
            coverUrl: 'http://qiniu.wkkwx.cn/FtVFmKwPQZsbhsGWQC7dJ_JG3UPq',
            industry: 499,
            isOnline: 'offline',
            startTime: 1619398800000,
            meetPriority: 1,
            roadshowType: ''
          },
          details: {
              ...
              name: ['yjh828719(新增)'],
              addTime: '2021/2/3/15时'
            },
            price: '免费',
            describe: [
              {
                text: [
                  '1、食品包装机械、纸箱包装机械、包装印刷机械；灌装机械，填充、封口、封箱、贴标机 械；立式、枕型包装机械、颗粒包装机械；真空、吸塑、贴体收缩包装机械；裹包、捆扎、 缠绕机械，商标、标签、喷码设备；不干胶印刷设备、包装检测设备、各种包装容器等。 2、各类食品加工机械、成套加工设备；面业及粮油加工机械和设备；糖果加工机械；；肉 类屠宰和加工机械；肉制品加工机械；方便食品加工机械；膨化类食品生产设备；调味品生 产设备；罐头成套设备；饮料和酒类生产设备；以及无菌罐装机、灭菌设备、干燥设备、保 鲜设备；冷冻、冷藏及保鲜技术和设备等； 3、包装材料及加工机械；绿色环保可降解包装材料及技术；包装防伪材料；食品用包装 袋、膜；注塑机、吹膜机等塑料加工机械；瓦楞纸生产线及单机、分切机、切纸机等。 4、检测技术及设备：化验仪器、检测设备、食品生产技术、生物工程、计算机应用技术、 制酸技术、酿造技术、水果蔬菜加工冷藏技术等；'
                ],
                title: '参展范围'
              }
            ],
            position: {
              address: ['410000', '410100', '410172'],
              xxaddress: '郑州国际会展中心'
            },
            view_vol: 220,
            sharelist: {
              url: 'https://zb-api.cuomj.cn/web/share',
              text: '郑州2021年食品机械展会定于2021年4月26-28日在郑州国际会展中心举办',
              thumb: 'http://qiniu.wkkwx.cn/FtVFmKwPQZsbhsGWQC7dJ_JG3UPq',
              title: '2021年郑州食品饮料机械设备展览会',
              sharetype: 'news'
            },
            follow_vol: 79,
            websiteUrl: 'http://www.chn-expo.com/exhibit/show-30140.html'
          },
          hide: 0
        }
      ],
  ...
  methods: {
    ...
    async vagueSelectss() {
      let params = {
        page: this.queryinfo.page,
        limit: this.queryinfo.limit,
        keyword: this.$refs.watchkeyword.mhcxData
      }
      const { data: res } = await getMeetList(params)
    ...
    async getSelect() {
      ...
      this.operateFormLable[10].opts = res.data[1].data
      this.operateFormLable[8].opts = res.data[2].data
    ...
    editData(row) {
      ...
      setTimeout(() => {
        // 查重表格的显示隐藏
        this.$refs.ruleForm.disablefhbtn('edit')
        // 消息内容的处理
        this.$refs.ruleForm.pushItems()
        // 是否为图片的勾选
        this.$refs.ruleForm.checkboxfun(row.details.describe)
        if (row.details.position.isAbroad == true) {
          this.$refs.ruleForm.isCityShowF(true)
        } else {
          this.$refs.ruleForm.isCityShowF(false)
        }
      }, 0)
      // dillog的title
    ...
    async adds() {
      ...
      const { data: res } = await addData(maininfo, details, this.forceSubmit)
      if (res.code !== 0 && res.code === 4) {
        this.Isforce = true
        this.ThisTimeSubData = [{ maininfo: maininfo, details: details }]
    ...
    async ContinueSub() {
      ...
      const { data: res } = await addData(
        this.ThisTimeSubData[0].maininfo,
        this.ThisTimeSubData[0].details,
        this.forceSubmit
      )
      if (res.code !== 0) return this.$message.error(res.msg)
      this.$message.success(res.msg)
